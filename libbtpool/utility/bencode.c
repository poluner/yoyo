...
{
    sd_assert(BNODETYPE(parent) == BNODE_DICT || BNODETYPE(parent) == BNODE_LIST);
...
PBNode bencode_free_node(PBNode node,PBNode parent,PBNode prev)
{
    ...
    {
        parent->sub = node->next;
    }

    if(node->sub)  //+应该释放sub避免泄露.
    {
        bencode_free_node_tree(node->sub);
    }
    if(node->rawtype&0x80)
    {
        free((char*)node->rawText);
    }
    free(node);
...
void bencode_free_node_tree(BNode *nodeTree)
{
    BNode *next;
    ...
    do
    {
        ...
        if(nodeTree->sub)bencode_free_node_tree((BNode *)nodeTree->sub);
        if(nodeTree->rawtype&0x80)
        {
            free((char*)nodeTree->rawText);
        }
        free(nodeTree);
        nodeTree = next;
    }
                    ...
                    int64_t *unit,  PBNode *subRoot, size_t &recursionLevel)
{
    ...
    *subRoot = NULL;
    ...
    for(/*x=0*/; x<len;/*x++*/)
    {
        pBNode = (BNode*)malloc(sizeof(BNode));
        if(pBNode==NULL)
        {
            ret = CRYPT_MEM;
            goto parse_error;
        ...
        case 'l':
            ...
            {
                //如果是顶层d或l，且不出现键值对情况，解析立即完毕.
                //在bt扩展协议包解析时触发
                sd_assert(pFirst==NULL);
                pFirst = pBNode;
        ...
        case 'i':
            ...
            while(in[x++]!='e');
            //pBNode->rawLen = 2; 之前的方法，因为rawText那里++过，第一个while的x++就可能越过len
            //while(in[x++]!='e')
            //{
            //    if(x>=len){
            //        ret = CRYPT_INVALID_PACKET;
            ...
            //填充num用的 以下为旧方法
            //memcpy(buf,pBNode->rawText+1,pBNode->rawLen-2);
            //buf[pBNode->rawLen++]=0;
            //sscanf(buf,"%lld",&pBNode->num);  //TODO 大数似乎有问题，例如 5864145935
        ...
        default:
            ...
            {
                pBNode->rawLen++;
                if(x>=len || pBNode->rawLen>MAX_NUMBER_STRING_LENGTH)
                {
                    ret = CRYPT_INVALID_PACKET;
                    free(pBNode);
                    pBNode = NULL;
            ...
            //以下为旧方法
            //memcpy(buf,pBNode->rawText,pBNode->rawLen);
            //buf[pBNode->rawLen++]=0;
            //sscanf(buf,"%lu",&length);
...
int _bencode_create(char type,int64_t num,const char *content, PBNode *created)
{
    ...
    case BNODE_BUF:
        ...
        prtlen = i64toa(num,bufnode,10);// sprintf(bufnode,"%lld:",num);
        bufnode[prtlen++]=':';
    ...
    *created = pBNode;
    return ret;
create_error:
    if(pBNode)free(pBNode);
    return ret;
}

int bencode_create_m(char type,PBNode *created) //used for 'l','d'
{
    assert(type==BNODE_DICT || type==BNODE_LIST);
...
{
    return _bencode_create(BNODE_BUF,len,(const char*)in,created);
}
...
{
    char buf[6];
    if(!pStart || pStart->type()!='d')return NULL;
    pStart = (BNode *)pStart->sub;
    while(pStart)
    {
        if(pStart->rawLen==6)
        {
            int x;
            memcpy(buf,pStart->rawText,6);
            for(x=0;x<6;x++)
            {
                if(buf[x]>='a')buf[x]=buf[x]-'a'+'A';
            }
            if(memcmp((const char*)buf,"4:INFO",6)==0)
            {
                return pStart->next;
            }
        }
        if(!pStart->next)break;	//'d'里面是键值对,所以跳两次
        pStart = pStart->next;
        if(!pStart->next)break;
        pStart = pStart->next;
    }
    return NULL;
}
...
{
    if(dict->type()!=BNODE_DICT || dict->sub==NULL || found==NULL)
    {
        return CRYPT_INVALID_ARG;
    }
    PBNode walk = dict->sub;
    do
    {
        if(walk->next==NULL)
        {
            return CRYPT_FUNC_FAILED;
        }
        if(walk->type() != BNODE_BUF)
        {
            return CRYPT_FUNC_FAILED;
        }
        if(memcmp(walk->content,key,keyLen)==0)
        {
            *found = walk->next;
            return CRYPT_OK;
        }
