...
{
    uint32_t p_value = (uint32_t)(uint64_t)ptr;
    ctx_md5 md5;
    uint8_t aeskey[16];
...
int32_t aes_decrypt_with_known_key(char* pDataBuff, uint32_t* nBuffLen, uint8_t *p_aeskey)
{
    int32_t ret;
    int nBeginOffset;
    char *pOutBuff;
...
int32_t sd_aes_encrypt( const char* aes_key,const uint8_t * src, uint32_t src_len,uint8_t * des, uint32_t * des_len)
{
    ...
    int32_t  nOutLen=0;					/*输出长度*/
    int32_t nInOffset=0;					/*进入加密偏移量*/
    int32_t nOutOffset=0;					/*完成加密偏移量*/
    ctx_aes	aes;
    ctx_md5	md5;
    unsigned char inBuff[ENCRYPT_BLOCK_SIZE],ouBuff[ENCRYPT_BLOCK_SIZE];
    char * key=NULL;
    uint8_t szKey[16];                                         /*密码: 16字节*/
    uint32_t total_out_buffer_len = *des_len;
    ...
    *des_len = 0;
    ...
    pOutBuff = (char*)malloc(src_len + ENCRYPT_BLOCK_SIZE);/*申请(len+16)长度作为输出串*/

    /*计算md5值，生成密码*/
    md5_initialize(&md5);//初始化md5
    md5_update_dl(&md5, (const unsigned char*)key, sd_strlen(key));
    md5_finish_dl(&md5, szKey);//完成md5计算，并将结果存入Key (密码: 16字节)
    ...
    while(TRUE)
    {
        ...
        else
        {
            ...
            int32_t nDataLen = src_len - nInOffset;//计算最后一段长度
            int32_t nFillData = ENCRYPT_BLOCK_SIZE - nDataLen;//最后一个加密段中需要填充的长度
...
int32_t sd_aes_decrypt(const  char* aes_key,const uint8_t * src, uint32_t src_len,uint8_t * des, uint32_t * des_len)
{
    ...
    char *pOutBuff=NULL;                                       /*输出串指针*/
    int32_t  nOutLen=0;					/*输出长度*/
    int32_t nInOffset=0;					/*进入解密偏移量*/
    int32_t nOutOffset=0;					/*完成解密偏移量*/
    ctx_aes aes;
    ctx_md5	md5;
    unsigned char inBuff[ENCRYPT_BLOCK_SIZE],ouBuff[ENCRYPT_BLOCK_SIZE];
    char * key=NULL;
...
{
    int32_t ret=0;
    uint64_t  nLastOutLen=0;					/*最后一段需要写入文件的长度*/
    uint64_t nInOffset=0;					/*进入解密偏移量*/
    uint64_t nOutOffset=0;					/*完成解密偏移量*/
    ctx_aes aes;
    ctx_md5	md5;
    unsigned char inBuff[ENCRYPT_BLOCK_SIZE],ouBuff[ENCRYPT_BLOCK_SIZE];
    char * key=NULL;
    ...
    char * out_ptr=NULL;
    char * old_file=NULL;
    char tmp_file[MAX_FULL_PATH_LEN] = {0};
    //char * system_path = em_get_system_path();
    uint32_t input_file_id=0, output_file_id=0,size_in=0,size_out=0;
    uint64_t file_size=0;
    char  buffer[2048];
    uint32_t buffer_len=2048, writesize=0;
    ...
    /*创建临时文件用来存储解密后的数据*/
    sd_snprintf(tmp_file, MAX_FULL_PATH_LEN-1, "%s.tmp",file_name);
